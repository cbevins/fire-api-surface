 ...
 */
export function reference (db, rh) {
 ...
 * @param {string} pos Correction for elevational diff from db & rh obs to site
 *  - 'below' indicates site is 1-2000 ft below the wx obs
 *  - 'above' indicates site is 1-2000 ft above the wx obs
 *  - 'level' indicates site is within 1000 ft of wx obs
 * @returns {number} 1-h fuel moisture correction (ratio)
 */
export function correction (month, shading, aspect, slope, hour, pos = 'level') {
  ...
  const posCat = (Position[pos] === undefined) ? 1 : Position[pos]
  // console.log(`mon:${month}=${monthCat} shade:${shading}=${shadeCat} asp:${aspect}=${aspectCat} slp:${slope}=${slopeCat} hr:${hour}=${hourCat} pos:${pos}=${posCat}`)
  return 0.01 * table[aspectCat][slopeCat][hourCat][posCat]
}
